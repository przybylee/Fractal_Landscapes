...
with $m\in\N\backslash\{1\}$, and $b_j\in \Z^d$ with 
\[\max_{1\le j\le k\le N}|b_j-b_k| \le  C.\]
(Choose the appropriate $C$.  Maybe )
...
\[L_\Psi\mathbf{f}_n = \mathbf{f}_n.\]
\end{theorem}
...
\[x_j = \frac{b_j}{m-1}.\]
This implies that for any two distinct fixed points, we have $|x_j-x_k|\ge (m-1)^{-1}$.
\begin{itemize}
